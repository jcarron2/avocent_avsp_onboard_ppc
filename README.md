...
And yes, I did brick a unit along the way (twice, actually) chasing a boot-region kernel-console patch that turned out not to be worth the risk -- recovered both times via the device's own TFTP network-recovery path, no hardware damage, just a couple of stressful hours. That's also how the flash chip part number above got confirmed -- opened the case during the recovery investigation to scope out physical reprogramming as a last-resort option, never actually needed it.
